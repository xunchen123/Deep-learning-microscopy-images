...
if __name__ == '__main__':
    ...
    def tkCNNw(event=None):
        ...
        if len(folder_config.get()) == 0 or len(folder_config1.get()) == 0:
            ...
            checkValue = [cv2.get(), cv3.get(), cv4.get(), c2.get()]
    ...
    v = root.entry('entry', key='<Return>', cmd=tkCNNw, focus=True, text= '256')
    v1 = root.entry('entry1', key='<Return>', cmd=tkCNNw, focus=True, text='256')
